...
- 美容カテゴリの予約リクエスト（受付・確定・取消）と、発注者・事業者ごとの状態管理
...
- Portal Planning Agentによるテーマ・地域・対象ポジション別の検索意図、既存コンテンツの被覆・不足検出、SEOページ生成、企画案・下書きへの適用
...
| 発注者 | 事業者検索、発注依頼、美容の予約リクエスト、依頼・予約状況、注文者向け情報 |
...
`FACT_CHECKED`は独立した状態ではなく、最新のファクトチェックとSEO監査を満たしたうえでレビュー・承認へ進みます。AIの出力は事実確認・レビュー・承認を経て公開します。IRや法務など正確性が重要な情報は、根拠と確認履歴を保持する前提です。
