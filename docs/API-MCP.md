...
| ポータル計画 | テーマ・地域・対象ポジションから検索意図、SEOページ案、既存コンテンツの被覆、不足情報、次アクションを生成 |
...
大量生成や外部ジョブからの実行に対応するため、事業者はコンテンツ作成を非同期ジョブとして投入できます。現在の対応操作は `content.create` です。投入時は `202` とジョブIDを返し、`operation.get` または `GET /api/v1/operations/{operationId}` で状態を取得します。Cloudflare Cronなどの外部スケジューラは `operation.execute_pending` または `POST /api/v1/operations/execute-pending` を呼び出してキューを処理できます。
...
| REST | MCP | 内容 |
|---|---|---|
| `POST /api/v1/operations` | `operation.submit` | 非同期コンテンツ作成ジョブを投入 |
