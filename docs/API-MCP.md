...
`content.propose`、`content.create`、`content.update`と対応するREST APIは、任意の`mediaIds`配列（最大20件）を受け付けます。CMS-OSは事業者のカテゴリ・事業者IDに一致するアセットだけを許可し、静的公開時には公開状態、公開URL、権利期限を再確認します。RESTとMCPは同じ`ContentService`を呼び出すため、所有者境界・入力制限・公開ゲートが一致します。
