...
ポータルの公開ロール（`user`、`orderer`、`provider`、`recruiter`）と、CMS内部ロール（`editor`、`hr`、`pr`、`ir`、`legal_reviewer`、`approver`、`publisher`、`partner_editor`、`partner_viewer`）は別管理です。内部割当が設定されたアカウントでは、承認・公開操作も内部ロールとカテゴリ／事業者スコープで検証します。
...
| ポータル表示 | カテゴリ・ロール別の表示体験、状態サマリー、次アクション |
...
GET  /api/v1/media?limit=50&cursor=0
POST /api/v1/media
POST /api/v1/media/seo-audit
GET  /api/v1/media/{assetId}
PATCH /api/v1/media/{assetId}
DELETE /api/v1/media/{assetId}
POST /api/v1/media/{assetId}/transform
POST /api/v1/media/{assetId}/seo-audit
GET  /api/v1/webhooks
POST /api/v1/webhooks
PATCH /api/v1/webhooks/{subscriptionId}
DELETE /api/v1/webhooks/{subscriptionId}
GET  /api/v1/webhooks/deliveries
POST /api/v1/webhooks/deliveries/deliver-pending
POST /api/v1/webhooks/deliveries/{deliveryId}/retry
POST /api/v1/webhooks/deliveries/{deliveryId}/deliver
POST /api/v1/operations
GET  /api/v1/operations?limit=50&cursor=0
GET  /api/v1/operations/{operationId}
POST /api/v1/operations/{operationId}/execute
POST /api/v1/operations/execute-pending
...
GET  /api/v1/content/{contentId}/editorial-actions
...
POST /api/v1/content/{contentId}/correction
POST /api/v1/content/{contentId}/withdrawal
...
コンテンツ作成・更新・訂正では、`structuredData`で会社・求人・PR・IRの型付き情報を、`sourceEvidence`で確認済み一次情報のタイトル・HTTPS URL・発行元・確認日を登録できます。`content.editorial_actions`は訂正前後の本文、ブロック、構造化データ、出典を返します。RESTとMCPは同じドメインサービスを通るため、どちらから操作しても版履歴・SEO監査・承認条件は同一です。
...
ログイン後の初期表示やAIエージェントの案内には、GET /api/v1/categories/{category}/summary と category.summary を利用します。metricsには現在のロールが見てよい公開事業者、外部案内、求人、依頼、予約、問い合わせ、応募、未読通知の集計だけを返し、nextActionsには現在のallowedActionsに含まれる導線だけを返します。発注者、事業者、リクルーターの集計は本人または自社に属するデータだけを対象にし、内部アカウントID、providerId、応募書類は含めません。カテゴリを切り替えた場合は、切り替え後のカテゴリのサマリーを再取得します。
...
| `content.list` / `GET /api/v1/content` | `search`、`status`、`audience`、`contentType`、`locale`、`visibility`、`tags`、`series`、`featured` | `updatedAt_desc`、`updatedAt_asc`、`title_asc`、`status` |
| `content.proposals` / `GET /api/v1/content/proposals` | `search`、`audience`、`contentType` | `createdAt_desc`、`createdAt_asc`、`topic_asc` |

コンテンツ一覧は、現在のカテゴリ・事業者に属するデータだけを対象に、対象ポジション（`audience`）、コンテンツ種別、言語、可視性、タグ、シリーズ、注目表示、ワークフロー状態をAND条件で絞り込みます。`tags`はRESTではカンマ区切り、MCPでは文字列配列です。AIエージェントは`page.nextCursor`を使って大量生成後の監査対象を分割取得できます。
...
カテゴリ別サマリーはMCP Resourceの categories/{category}/summary からも読み取れます。AIエージェントは認証ロールに応じた件数と次アクションを先に確認してから、求人、依頼、予約、コンテンツ操作を提案します。

例として、次のツールを提供する。

- `content.propose`
- `content.create`
- `operation.submit`（`content.create_batch`などの一括操作）
- `webhook.list`
- `webhook.create`
- `webhook.update`
- `webhook.revoke`
...
大量生成や外部ジョブからの実行に対応するため、事業者はコンテンツ操作を非同期ジョブとして投入できます。`content.create`は単一登録、`content.create_batch`は検証済み本文の一括登録、`content.propose_batch`は対象ポジション別の企画案一括生成、`content.draft_batch`は企画案から下書きの一括生成、`content.polish_batch`は下書きの清書一括生成、`content.prepare_batch`は企画・下書き・清書・事実確認・SEO監査までの一括準備に対応し、それぞれ同一カテゴリの1〜50件を1ジョブにまとめて処理します。`content.polish_batch`と`content.prepare_batch`には全対象へ適用する任意の`instructions`を指定できます。`content.prepare_batch`は承認・公開を実行せず、監査結果を確認できる状態で停止します。投入時は `202` とジョブIDを返し、`operation.get` または `GET /api/v1/operations/{operationId}` で状態を取得します。バッチ成功時は`result.contentIds`、`result.proposalIds`、または`result.items`、途中失敗時は作成済みIDと`completedCount`を返します。`content.prepare_batch`の`result.items`は各入力の`inputIndex`と段階状態を保存するため、AI呼び出し途中の失敗でも企画案・下書き・清書を重複作成せず、最後に保存された段階から再開します。Cloudflare Cronなどの外部スケジューラは `operation.execute_pending` または `POST /api/v1/operations/execute-pending` を呼び出してキューを処理できます。

`Idempotency-Key` を指定すると、同じ事業者・操作・キー・入力の再送は既存ジョブを返します。同じキーで異なる入力を送ると `409` を返します。失敗したバッチジョブを`operation.execute`または`operation.execute_pending`で再実行すると、保存済みの`completedCount`と生成済みIDから再開し、完了済みの企画・下書き・清書を重複作成しません。ジョブの入力本文は状態取得結果に含めず、カテゴリとproviderIdで所有者を分離します。
...
`content.propose`、`content.create`、`content.update`と対応するREST APIは、任意の`mediaIds`配列（最大20件）を受け付けます。`content.create`と`content.update`はnote型の`blocks`配列（見出し、段落、画像、表、CTA、求人・PR・IRカードなど）も受け付けます。CMS-OSはブロックを検証してMarkdown本文へ正規化し、事業者のカテゴリ・事業者IDに一致するアセットだけを許可し、静的公開時には公開状態、公開URL、権利期限を再確認します。RESTとMCPは同じ`ContentService`を呼び出すため、所有者境界・入力制限・公開ゲートが一致します。
