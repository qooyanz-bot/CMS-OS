...
- 計画反映: ページ案を対象ポジション別の`ContentProposal`へ一括変換し、`content.draft`以降の編集フローへ渡せます。`portal.plan.draft`を使うと企画案から下書きまで冪等に作成できます。
...
- 外部AIを接続する場合は`ContentAgentAdapter.planPortal`（HTTPでは`portal_plan`）へ計画生成を委譲できます。カテゴリ権限、件数、ページパス、検索意図、SEO公開条件の検証と計画保存はCMS-OSが継続して担います。
...
| `POST /api/v1/portal-plans/{planId}/draft` | `portal.plan.draft` | 計画から企画案と下書きを冪等に作成 |
...
`portal.plan.apply`を一度実行すると計画に作成済み企画案IDと反映日時が記録されます。同じ計画を再度反映しても企画案は重複作成されません。`portal.plan.draft`は既存の企画案に紐づく下書きを再利用し、計画に下書きIDと作成日時を記録します。求人ページは`job`・`candidate`、その他のページは計画の`audience`を引き継ぎます。
