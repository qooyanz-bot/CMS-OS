...
AI編集のモデル依存部分は`src/integrations/content-agent-adapter.ts`の`ContentAgentAdapter`に分離しています。アダプターは次の4処理を実装します。

| メソッド | 入力 | 出力 | CMS-OSが引き続き担う処理 |
|---|---|---|---|
| `propose` | カテゴリ、対象ポジション、テーマ、検索意図、一次情報 | 検索意図、関連語、見出し構成、企画理由 | 所有権、入力制限、企画案保存 |
| `draft` | 保存済み企画案、対象ポジション、JSON-LD種別 | タイトル、要約、本文、SEO候補 | slug、canonical、SEO監査、版保存 |
| `polish` | 保存済みコンテンツ、清書指示 | 清書本文、任意のタイトル・要約・SEO候補 | 状態遷移、版保存、権限、監査 |
| `translate` | 原文、翻訳先言語、翻訳指示 | 翻訳タイトル、要約、本文、SEO候補 | 原文との関連付け、canonical、重複防止、公開ゲート |
...
HTTPリクエストは次の形式です。AIプロバイダーは`output`へ、対象操作に対応したJSONを返します。APIキーは本文やログへ含めません。
