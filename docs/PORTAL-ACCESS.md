...
カテゴリ別サマリーは、GET /api/v1/categories/{category}/summary またはMCPのcategory.summaryで取得します。ユーザーには公開事業者・外部案内、発注者には自分の依頼・美容カテゴリの予約、事業者には自社の依頼・求人・問い合わせ・応募、リクルーターには公開求人・自分の応募を投影します。未読通知は本人または自社事業者のものだけを集計します。nextActionsはカテゴリポリシーのallowedActionsに基づくため、UIやAIエージェントが存在しない操作を提案しません。
...
- カテゴリ・ロール別ナビゲーション：APIから返された`navigation`だけを画面上部に表示し、公開検索、依頼・予約、管理、求人・応募など該当パネルへ移動
- お気に入り一覧：ユーザー、発注者、リクルーターが現在カテゴリの公開事業者を保存・解除
- 事業者プロフィール詳細：一覧からAPIのロール別投影を取得し、公開項目・発注者向け項目・リクルーター向け項目を権限に応じて表示
- 発注者の事業者プロフィール詳細：`request.create`が許可された場合だけ「この事業者へ依頼」を表示し、依頼フォームの事業者を自動選択
...
- AIコンテンツ手動編集：事業者が生成済みコンテンツの本文、一次情報、SEO、JSON-LD、FAQを確認・修正し、保存後に再監査
...
- 既定のファイルストレージはDBなしのローカル運用向けです。揮発性のテスト・短時間デモでは`CMS_OS_STORAGE=memory`を指定し、複数プロセス・本番運用ではPostgreSQLへ切り替えます。
