...
- 미용 카테고리의 예약 요청(접수·확정·취소)과 역할별 상태 관리
- 콘텐츠 검토, 승인, 공개, 비공개, 버전 이력
- 이미지·동영상·PDF 관리, alt 텍스트·구조화 데이터·내부 링크·SEO 감사
- BuilderOS Adapter를 통한 정적 사이트 생성 및 Cloudflare Pages 공개
- Portal Planning Agent를 통한 주제·지역·대상 포지션별 검색 의도 분석, 기존 콘텐츠 커버리지 부족 탐지, SEO 페이지 생성, 콘텐츠 기획·초안 적용
...
| 발주자 | 사업자 검색, 발주 요청, 미용 예약 요청, 요청·예약 상태, 발주자 정보 |
