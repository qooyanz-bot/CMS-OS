...
describe("CMS-OS承認済み静的公開", () => {
  it("承認前の公開を拒否し、SEOアセット付きCloudflare向けファイルを生成する", async () => {
    ...
    assert.match(fileMap.get("sitemap.xml") ?? "", /<lastmod>2026-07-01T00:00:00\.000Z<\/lastmod>/);
    ...
    assert.match(fileMap.get("categories/legal/jobs/job-legal-demo/index.html") ?? "", /CMS-OS法律事務所（サンプル）/);
    assert.match(fileMap.get("categories/legal/jobs/job-legal-demo/index.html") ?? "", /datePosted/);
    assert.match(fileMap.get("categories/legal/jobs/job-legal-demo/index.html") ?? "", /dateModified/);
